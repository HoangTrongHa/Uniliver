...
useAsyncData('fetch', async () => {
    try {
        dataPage.value = await useApiFetch('brands.section', {code: 'nutrition'});
    ...
    <div class="font-montserrat-alternates">
        ...
        <section class="sm:px-14 px-4 sm:py-[40px] py-10 bg-green-20 relative">
            ...
            <div
                ...
                <card-time class="z-10 relative" v-for="item in posts" :key="item.title" :image="item.image"
                    :title="item.title" title-class="leading-[24px] sm:mt-[15px] mt-0 font-[600]" :date="item.date"
                    image-class="sm:w-full sm:h-auto w-[343px] h-[200px]" :limit-row-title="3">
                    <span class="absolute top-0 left-0 p-[8px] text-white text-xs leading-[normal] sm:hidden block"
                        :class="categoryOptions[item.categoryId]">
                        {{ item.categoryName }}
                    </span>
                </card-time>
            </div>
            <div class="absolute top-0 bottom-0 left-0 right-0">
                <img class="w-full h-full" :src="useAsset('images/brands/subcategory/Vector_bg.png')"
                    :alt="useAsset('images/brands/subcategory/Vector_bg.png')">
            </div>
